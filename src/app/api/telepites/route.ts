...
import { requireToRole } from "@/lib/guards";
...
export async function POST(req: Request) {
    ...
    const auth = await requireToRole(toAzonosito, "STAFF");
    if (!auth.ok) {
    ...
    try {
        const eredmeny = await prisma.$transaction(async (tx) => {
            ...
            // 5) napló
            const leirasReszek = [
                `Telepítés: ${to.nev} – ${halfaj.nev}`,
                `${darab} db`,
                `tömeg: ${minTomegKg || 0}-${maxTomegKg || 0} kg`,
            ];

            if (forras) {
                leirasReszek.push(`forrás: ${forras}`);
            }
