...
- **Védelem:** `NaploEsemeny` auditnapló minden fontos műveletnél, idő­bélyeggel és
  leírással; áttelepítésnél forrás+cél esemény is keletkezik.
- **Reziduális kockázat:** a napló **nem rögzíti a cselekvő felhasználó azonosítóját**
  (csak tó/halfaj/darab/leírás). Egy adott eseményt nem lehet konkrét userhez
  kötni. TODO: `felhasznaloId` felvétele a naplóba.
